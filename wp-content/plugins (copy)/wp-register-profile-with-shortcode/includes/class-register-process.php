...
class Register_Process {
	...
	public function create_user( $data = array() ){
		...
		$userdata['first_name'] = $data['userdata']['display_name'];
		
		// insert new user in db //
			$user_id = wp_insert_user( $userdata );

			$userdata['alg_wc_ev_is_activated'] = "1";

			update_user_meta( $user_id, 'alg_wc_ev_is_activated', $userdata['alg_wc_ev_is_activated'] );
		// insert new user in db //
