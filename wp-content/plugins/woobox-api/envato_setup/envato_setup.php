...
require_once(WOO_DIR . 'envato_setup/tgm/woobox-required-plugins.php');

//require_once(WOO_DIR . 'envato_setup/TGM-plugin/example.php');
...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function init_globals() {
			...
			$this->page_slug       = apply_filters( $this->theme_name . '_theme_setup_wizard_page_slug', 'woobox-setup' );
		...
		public function init_actions() {
			if ( apply_filters( $this->theme_name . '_enable_setup_wizard', true ) && current_user_can( 'manage_options' ) ) {
				...
				if ( class_exists( 'Woobox_TGM_Plugin_Activation' ) && isset( $GLOBALS['tgmpa'] ) ) {
		...
		public function admin_menus() {
			...
			if ( $this->is_submenu_page() ) {
				...
				add_menu_page( esc_html__( 'Setup Wizard' ), esc_html__( 'Woobox Setup' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			}
		...
		public function setup_wizard() {
			...
			wp_register_script( 'jquery-blockui', WOO_DIR_URI . 'envato_setup/js/jquery.blockUI.js', array( 'jquery' ), '2.70', true );
			wp_register_script( 'envato-setup', WOO_DIR_URI . 'envato_setup/js/envato-setup.js', array(
				'jquery',
				'jquery-blockui',
			), $this->version );
			...
			wp_enqueue_style( 'envato-setup', WOO_DIR_URI . 'envato_setup/css/envato-setup.css', array(
				'wp-admin',
				'dashicons',
				'install',
			), $this->version );
			...
			wp_enqueue_style('all-min', WOO_DIR_URI .'envato_setup/css/all.min.css',array(), '4.1.3', 'all');
	...
	public function setup_wizard_header() {
		...
		<head>
			...
			echo '<t';
			echo 'itle>' . esc_html__( 'Woobox Api Plugin &rsaquo; Setup Wizard' ) . '</ti' . 'tle>'; ?>
		...
		<h1 id="wc-logo">
			<img src="<?php echo esc_url( WOO_DIR_URI . 'envato_setup/images/logo.png' ); ?>" />		
		</h1>
		<?php
		}
		...
		<?php if ( 'next_steps' === $this->step ) : ?>
			<h3><a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard' ); ?></a></h6>
		<?php endif; ?>
		</body>
		<?php
		...
		public function envato_setup_introduction() {
			...
			if ( false && isset( $_REQUEST['debug'] ) ) {
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for WooBox APi' )); ?></h1>
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for WooBox APi' )); ?></h1>
				<p><?php printf( esc_html__( 'Thank you for choosing the Woobox Api. This quick setup wizard will help you configure your plugin. This wizard will install the required WordPress plugins and tell you a little about Help &amp; Support options. It should only take 5 minutes.' ) ); ?></p>
				
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e( 'Configure' ); ?></a>
		...
		public function envato_setup_default_plugins() {
			...
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					...
					<p><?php esc_html_e( 'Do not Refresh The Page Untill All plugins Installed.' ); ?></p>
					<?php
				} else {
					echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.' ) . '</strong></p>';
				} ?>

				<p><?php esc_html_e( 'Do not Remove Above Plugins' ); ?></p>
		...
		public function envato_setup_ready() {
			...
			<h1><a><i class="fa fa-check"></i><a><?php esc_html_e( 'Your Setup is Ready!' ); ?></h1>

			<p>All plugins have been successfully installed.</p>
			 <p>Follow the next steps from the document to complete your setup.
			</p>
